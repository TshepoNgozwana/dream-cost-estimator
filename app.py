...
# Cockpit logger
# ──────────────────────────────────────────────────────────────
def _log(file: Path, event: str, payload: Dict[str, Any]):
    entry = {
...
if "trial_days_left" not in st.session_state:
    st.session_state.trial_days_left = 3  # 3-day demo
...
# ──────────────────────────────────────────────────────────────
# Cost model (simple; internal markup applied)
# ──────────────────────────────────────────────────────────────
...
def load_openai_probe() -> Dict[str, Any]:
    ...
    try:
        ...
        reg_ok = False
    return {"folder": ok_folder, "has_key": has_key, "registry": reg_ok}
...
def milkbot_tab():
    ...
    if "milkbot_messages" not in st.session_state:
        st.session_state.milkbot_messages: List[Dict[str, str]] = [
...
# ──────────────────────────────────────────────────────────────
# Dream page (your existing UI, unchanged)
# ──────────────────────────────────────────────────────────────
def dream_tab():
    with st.sidebar:
        st.header("Setup")
        ...
        st.divider()
        debug_skip_redirect = st.checkbox(
            "🛠️ Debug: Skip redirect to MilkBoxAI", value=False,
        ...
        with st.expander("📦 Universal Uploads", expanded=False):
            ...
            if files:
                ...
                _log(LOG_DREAM, "uploads.saved", {"files": saved})
        ...
        with st.expander("🛫 Preflight Check", expanded=True):
            ...
            if not checks["API key detected"]:
                st.info("Tip: Create a `.env` file with `OPENAI_API_KEY=sk-...`", icon="ℹ️")
    ...
    with left:
        ...
        # Offline heuristic structuring (no network)
        if do_autostruct:
            ...
            _log(LOG_DREAM, "autostructure", {"prompt_len": len(text)})
            st.success("Structured draft filled in below. Edit in Advanced if needed.", icon="✅")
        ...
        if st.button("Generate Brief & Log", use_container_width=True):
            _log(LOG_DREAM, "dream.submit", {"category": category, "answers": st.session_state.answers})
    ...
    with right:
        ...
        with col_trial[0]:
            st.info(f"⏳ {st.session_state.trial_days_left} days left · 🔑 {st.session_state.trial_tokens} tokens", icon="ℹ️")
        with col_trial[1]:
            if st.button("Use 1 Token", use_container_width=True, disabled=st.session_state.trial_tokens <= 0):
                if st.session_state.trial_tokens > 0:
                    ...
                    _log(LOG_DREAM, "trial.consume", {"left": st.session_state.trial_tokens})
                    st.toast("Token used.")
                else:
        ...
        if ok_to_deploy:
            if not name or not email:
                ...
                _log(LOG_DREAM, "handoff.deploy", payload)
                st.success("Queued for War Room.", icon="✅")
                if not st.session_state.get("debug_skip_redirect", False):
                    st.caption("Redirect suppressed in this skeleton. (Use Debug toggle in sidebar).")

    st.markdown("---")
    st.subheader("Cockpit Activity Log (raw)")
    if LOG_DREAM.exists():
        lines = LOG_DREAM.read_text(encoding="utf-8").splitlines()[-50:]
        for ln in lines:
            st.code(ln)
    else:
